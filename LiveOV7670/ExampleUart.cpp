...
void processFrame() {
  ...
  interrupts();
  frameCounter++;
  commandDebugPrint("Gambar " + String(frameCounter)/* + " " + String(processedByteCountDuringCameraRead)*/);
...
void processGrayscaleFrameBuffered() {
  camera.waitForVsync();
  commandDebugPrint("Gerak Terdeteksi");
